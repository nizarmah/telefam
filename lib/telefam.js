...
// Start - Changes : July 21, 2017

Telefam.prototype.getStickerSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "getStickerSet?name=" + squad.name;

	return this._sendRequest(url, callback);
};

Telefam.prototype.uploadStickerFile = function (squad, callback){
	var callback = callback || function (error, res) { return ((error) ? error : res); };
	var url = this.url + 'uploadStickerFile';
	var formData = {  };

	formData.user_id = squad.user_id;

	if (squad.certificate != null)
		formData.png_sticker = fs.createReadStream(squad.png_sticker);

	return this._sendForm(url, formData, callback);
};

Telefam.prototype.createNewStickerSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); };
	if (typeof squad.png_sticker == "object") {
		if (squad.png_sticker.path != null && squad.png_sticker.url == null) {	
			var url = this.url + "createNewStickerSet";
			var formData = {  };

			formData.user_id = squad.user_id;
			formData.name = squad.name;
			formData.title = squad.title;
			formData.emojis = squad.emojis;

			if (squad.contains_mask != null && squad.mask_position != null) {
				formData.contains_mask = squad.contains_mask;
				formData.mask_position = squad.mask_position;
			}

			formData.png_sticker = fs.createReadStream(squad.png_sticker.path);

			this._sendForm(url, formData, callback);
		} else if (squad.png_sticker.path == null && squad.png_sticker.url != null) {
			var url = this.url + "createNewStickerSet?user_id=" + squad.user_id + "&name=" + squad.name + "&title=" + squad.title + "&png_sticker=" + squad.png_sticker.url + "&emojis=" + squad.emojis;

			if (squad.contains_mask != null && squad.mask_position != null)
				url = url + "&contains_mask=" + squad.contains_mask + "&mask_position=" + squad.mask_position;

			this._sendRequest(url, callback);
		}
	} else {
		var url = this.url + "createNewStickerSet?user_id=" + squad.user_id + "&name=" + squad.name + "&title=" + squad.title + "&png_sticker=" + squad.png_sticker + "&emojis=" + squad.emojis;
...
Telefam.prototype.addStickerToSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); };
	if (typeof squad.png_sticker == "object") {
		if (squad.png_sticker.path != null && squad.png_sticker.url == null) {	
			var url = this.url + "addStickerToSet";
			var formData = {  };
			...
			var url = this.url + "addStickerToSet?user_id=" + squad.user_id + "&name=" + squad.name + "&png_sticker=" + squad.png_sticker.url + "&emojis=" + squad.emojis;

			if (squad.mask_position != null)
				url = url + "&mask_position=" + squad.mask_position;

			this._sendRequest(url, callback);
		}
	} else {
		var url = this.url + "addStickerToSet?user_id=" + squad.user_id + "&name=" + squad.name + "&png_sticker=" + squad.png_sticker + "&emojis=" + squad.emojis;
...
Telefam.prototype.setStickerPositionInSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "setStickerPositionInSet?sticker=" + squad.sticker + "&position=" + squad.position;

	return this._sendRequest(url, callback);
};

Telefam.prototype.deleteStickerFromSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "deleteStickerFromSet?sticker=" + squad.sticker;

	return this._sendRequest(url, callback);
};

// End - Changes : July 21, 2017

// Start - Changes : October 11, 2017

Telefam.prototype.setChatStickerSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "setChatStickerSet?chat_id=" + squad.chat_id + "&sticker_set_name=" + squad.sticker_set_name;

	return this._sendRequest(url, callback);
};

Telefam.prototype.deleteChatStickerSet = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "deleteChatStickerSet?chat_id=" + squad.chat_id;

	return this._sendRequest(url, callback);
};

Telefam.prototype.editMessageLiveLocation = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "editMessageLiveLocation?latitude=" + squad.latitude + "&longitude=" + squad.longitude;

	if (squad.chat_id != null || squad.message_id != null) {
		if (squad.chat_id != null) {
			url = url + "?chat_id=" + squad.chat_id;

			if (squad.message_id != null)
				url = url + "&message_id=" + squad.message_id;
		} else {
			if (squad.message_id != null)
				url = url + "?message_id=" + squad.message_id;
		}
	} else if (squad.inline_message_id != null)
		url = url + "?inline_message_id=" + squad.inline_message_id;
...
Telefam.prototype.stopMessageLiveLocation = function (squad, callback) {
	var callback = callback || function (error, res) { return ((error) ? error : res); }
	var url = this.url + "stopMessageLiveLocation";
