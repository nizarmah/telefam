...
Telefam.prototype.setWebhook = function (squad, callback){
    ...
    if (squad.certificate != null) formData.certificate = fs.createReadStream(squad.certificate);

    this._sendRequest(url, callback);
    theThis = this;
    return this._setupForm(url, formData, null, function (url, formData) { theThis._sendForm(url, formData, callback); });
};
